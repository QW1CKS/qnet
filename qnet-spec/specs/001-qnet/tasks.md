...
- [ ] T6.7: Stealth Browser Application
    ...
    - Added masked connection smoke test (PowerShell) documentation in root `README.md` and `qnet-spec/docs/helper.md` (Task: T6.7-smoke-masked). Acceptance: one-liner executes, `/status` becomes `connected`, curl HEAD succeeds via SOCKS5.
    - [x] T6.7-remote-catalog-verify: Remove embedded publisher.pub; require explicit STEALTH_DECOY_PUBKEY_HEX; update `scripts/test-masked-connect.ps1` to fetch remote signed catalog (qnet-catalog seed), verify signature (catalog-signer verify), export env vars, and abort on verification failure. Acceptance: running script with valid pubkey + remote catalog yields `Verification=signature-ok` in summary, status shows decoy_count > 0, and build succeeds without assets/publisher.pub file. Security: no unsigned fallback in this path.
    - [x] T6.7-prod-catalog-initial: Replace template/example decoy hosts with production set (google, amazon, wikipedia, g2a); update `decoys.yml`, `catalog.meta.yml`, and `catalog.json` placeholders (signature removed) for re-signing; remove `<org>` and `example` strings. Acceptance: signer produces new inline-signed catalog with entries=4, verify passes with new pubkey.
    - [x] T6.7-masked-decoy-robustness: Harden masked connect test and runtime catalog precedence. Changes: (1) `scripts/test-masked-connect.ps1` adds safe optional field extraction (no crash when `last_target` absent), summary of masked attempt counters, `-UseLocalEdgeDecoys` switch with local cert trust + optional plaintext inner mux for debugging; (2) `stealth-browser` `load_decoy_catalog_signed_or_dev` now prioritizes env-provided catalog (verified earlier) over template files (avoids template shadowing derived catalogs); (3) added HTX_TRUST_PEM + HTX_INNER_PLAINTEXT support path in script for local edge TLS/self-signed dev; (4) logged env-precedence load. Acceptance: running script without successful CONNECT yields graceful summary (no property exception) and decoy_count reflects derived entries=4. Security: unsigned derivation still gated by prior signature verification step.
