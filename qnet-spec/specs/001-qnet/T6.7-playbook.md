...
Status: In progress (M1 complete; M2 next — stealth networking)
...
- [x] Implement feature flag: htx/stealth-mode
  ...
  - Behavior: when on, enable variable record sizing and bounded jitter in HTX mux write path; ALPN/JA3 shaping and padding wiring next
- [ ] TLS-like record sizing and padding profiles (small, webby, bursty)
  ...
  - [ ] Padding: right-pad to chosen size before AEAD; zeroize temp buffers; tests
  - [ ] Tests: distribution sanity (CDF buckets), AEAD round-trip with padding
- [ ] Timing jitter (bounded) for inter-record gaps
  ...
  - [ ] Tests: determinism with fixed seed; bound checks
- [ ] ALPN shaping and JA3-aligned client hello templates
  ...
  - Rotation: per-conn template rotation among a signed allow-list; configurable cadence (time/volume)
  - ALPN order preserved from template; enforce extension order
  - Tests: template ID stability; JA3 hash parity vs reference fixture
- [ ] QUIC mode parity: stream scheduling + flow-control defaults for web traffic
  - Add `prefer_quic` config; when QUIC enabled (future), align stream concurrency to HTTP/2-like patterns
  - Scheduler: weight smaller streams, coalesce small writes; window defaults tuned for TTFB
  - Tests: synthetic workloads with mixed small/large objects show improved p50/p95
- [ ] Configurable decoy routing (domains/IPs) with rotation policies
  - Schema: signed JSON/CBOR { version, updated_at, entries: [ {host, alpn, port, weight} ], signature }
  - Signature: Ed25519 over deterministic CBOR; key pinned in binary for M2
  - Resolver: map real destination → decoy per policy (time-based or bytes-based rotation)
  - Logs: surface only decoy host/IP when enabled (plausibility)
  - Tests: signature verify; resolver rotation; log redaction unit test
- [ ] Bootstrap seed list + fallback + backoff + caching
  - Seeds: signed list (same mechanism as decoys) with health endpoints
  - Backoff: 0.5s → 8s exponential with jitter; cap 30s; cache good seeds for 24h
  - Acceptance timing target: first successful tunnel <30s on commodity link
  - Tests: simulated failures and eventual success path with bounded total time
...
- Record sizing & padding
  ...
  - Padding is next: right-pad payload prior to AEAD and maintain AAD length integrity; ensure zeroization.
- Timing jitter
...
- ALPN/JA3 shaping
  - Validate with Wireshark JA3 plugin and captured pcaps from Chrome/Firefox baselines.
  - Keep a minimal signed template catalog; prefer dynamic calibration where allowed.
- Decoy routing
  - Start with DNS-over-OS; future: DoH/DoT integration if required by networks.
  - Keep resolver optional; when off, logs must not expose real destinations beyond localhost.
- Bootstrap
