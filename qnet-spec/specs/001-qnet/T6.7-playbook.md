...
Status: In progress (M1 complete; M2 repo-local acceptance done: external seeds + DPI parity; QUIC parity evidence added; M3 catalog pipeline complete: signer CLI + signed artifacts verified; app-side loader/verify/cache/updater and status surfacing added; Windows validation successful)
...
- DecoyDirect demo landed: SOCKS DOMAIN requests can be masked by decoys from the catalog; `/status` exposes `last_decoy`. A PowerShell script `scripts/test-decoy-rotation.ps1` demonstrates rotation and masking for configured domains.
...
- [x] Updater from mirrors
  ...
  - [ ] Add a "Check for updates" trigger; backoff and error logging
    - Evidence: `CatalogState::updater` polls `update_urls`, verifies, and swaps on strictly higher `catalog_version`.
- [ ] Connected semantics (seedless online)
  - [ ] Mark state=connected after the first successful SOCKS CONNECT (no seeds required); surface in `/status` and logs
  - [ ] Document semantics in `qnet-spec/docs/apps/stealth-browser.md` and reference from the playbook
...
- [x] DecoyDirect demo and masking acceptance (repo-local)
  - [x] Implement DecoyDirect routing for SOCKS DOMAIN requests using the signed decoy catalog
  - [x] Expose `last_decoy` in `/status`; add `scripts/test-decoy-rotation.ps1` to demonstrate rotation for configured domains
  - [ ] Plan: expand decoy catalog for common masking pairs (e.g., google.com → youtube.com) and add CI artifacts alongside the main catalog
- [ ] Publisher repo & CI
  - [ ] Create `qnet-catalogs` repo and add build-sign-publish workflow to produce `dist/catalog.json` (+ `.sig`)
  - Note: A repo-local workflow is available as a template: `.github/workflows/catalog-publish.yml` (uses `crates/catalog-signer`).
  - [ ] Document pubkey fingerprints and rotation in README/UI
  - Note: For now, manually sign catalogs locally using `catalog-signer sign` with `CATALOG_PRIVKEY` set to your private seed (64-char hex). Store the seed securely (e.g., in a password manager). When QNet gets big, automate this in CI with GitHub Secrets to avoid local seed exposure.

Exit: Catalog-first path working end-to-end (bundled+signed + updater + fallback) and surfaced in a minimal status API.
...
- [ ] Publisher repo & CI workflow documented
  - Evidence: `docs/catalog-publisher.md` with workflow sketch and security notes
- [ ] Catalog-first path integrated in app (bundled + updater + status API)
  - Evidence: App builds with embedded catalog; updater fetches from mirrors; status API exposes source/version/expiry
