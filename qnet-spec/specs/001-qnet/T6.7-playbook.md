...
Status: In progress (M1 complete; M2 repo-local acceptance done: external seeds + DPI parity; QUIC parity evidence added; M3 catalog pipeline progressing: signer CLI + signed artifacts verified; app-side loader/verify/cache/updater and status surfacing added)
...
- [ ] Bundle catalog in app
  - [ ] Add `assets/catalog-default.json` (+ `.sig`) to `apps/stealth-browser` and embed at build
