...
- [ ] Catalog signer and templates
  - [ ] Create `catalog-signer` (Rust bin): YAML → JSON → DET-CBOR → Ed25519 (detached) signature
  - [ ] Author `/templates/decoys.yml` and `/templates/catalog.meta.yml` (schema_version, ttl, publisher_id, update_urls)
  - [ ] Emit `catalog.json` and `catalog.json.sig`; verify with pinned pubkey
