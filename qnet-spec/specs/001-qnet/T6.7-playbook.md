...
The recommended production deployment is a Browser Extension + Helper model. The extension exposes UI and user controls and communicates with a local Helper service (the `stealth-browser` binary) via native messaging or the Helper's local status API. Helper defaults used in examples:

- SOCKS5: `127.0.0.1:1088`
- Status API: `http://127.0.0.1:8088`

This model is preferred for initial user distribution and reduces packaging complexity versus shipping a full Tauri desktop UI.
...
## objective
Build a user-facing Browser Extension + Helper experience that:
...
- Ops: Helper installers for Windows/macOS/Linux with auto-launch; extension store listings.

---

## deliverables
- Browser Extension (UI)
  - Popup UI: start/stop toggle, status, settings
  - Background: proxy settings management; Helper IPC (native messaging preferred)
  - Packaging: Chrome/Edge/Firefox WebExtension with native messaging host manifest
- Helper service (the `stealth-browser` binary)
  - Local SOCKS5 proxy bound on 127.0.0.1:1088 (default)
  - Local HTTP status/control API on 127.0.0.1:8088 (default) for dev and diagnostics
  - Catalog verification and updater (signed catalog, mirrors)
  - Optional edge-gateway launcher for masked mode (prod)
...
## architecture
- UI: Browser Extension (WebExtension)
- IPC: Native Messaging (production) or localhost HTTP/WS (dev) to the Helper
- Network path: Browser -> local SOCKS5 (Helper) -> HTX tunnel -> QNet peers
...
- Config
  - Decoy catalog (signed) bundled with the Helper (or shipped alongside) and cached on disk with TTL; background updater from mirrors
  - Bootstrap seeds + backoff plan retained as a resilience fallback
  - Feature flags: stealth-mode, prefer-quic, measure-latency

---

## Troubleshooting (dev, Windows)

Common symptoms and quick fixes when running the local Helper + Edge Gateway:

- Symptom: `socks client ... htx dial failed: FeatureDisabled`
  - Cause: The `htx` crate wasn’t built with the `rustls-config` feature, so `htx::api::dial()` is compiled out.
  - Fix: Clean and rebuild the Helper to ensure features are enabled via its dependency:
    - The Helper (`apps/stealth-browser`) depends on `htx` with `features = ["rustls-config", "stealth-mode"]`. If you built previously before this wiring, do a targeted clean then rebuild.

- Symptom: `curl --socks5-hostname 127.0.0.1:1088 ...` fails, but `/status` shows `"socks_addr": "127.0.0.1:1080"`
  - Cause: You’re connecting to the wrong SOCKS port (your current binary is using the legacy default 1080).
  - Fix: Either connect to the port shown in `/status` or set `STEALTH_SOCKS_PORT=1088` before starting the Helper. After a rebuild (newer default), the Helper will bind `127.0.0.1:1088` by default and the status server to `127.0.0.1:8088`.

- Symptom: Masked mode with a local Edge still doesn’t work
  - Fix: In dev, set `STEALTH_DECOY_DEV_LOCAL=1` before running the Helper. This sends all decoy traffic to `https://localhost:4443` and adds `certs/edge.crt` to the trust bundle automatically when present. Ensure the Edge gateway is running with `BIND=0.0.0.0:4443` and `HTX_TLS_CERT/HTX_TLS_KEY` set.

- Symptom: TLS error InvalidCertificate(Other(CaUsedAsEndEntity)) or server logs `AlertReceived(CertificateUnknown)`
  - Cause: The certificate used by the Edge is marked as a CA (CA:TRUE) but is presented as an end-entity/leaf cert. Clients will reject this.
  - Fix (preferred): Use a proper leaf certificate signed by a CA and trust the CA root on the client.
    - Edge: present a leaf (end-entity) cert, e.g. `certs/localhost.pem` with key `certs/localhost-key.pem`. Do NOT set CA:TRUE on the leaf.
    - Helper: set `HTX_TRUST_PEM` to the CA root that signed the leaf (not the leaf). If you generated a dev root (e.g. `certs/dev-rootCA.pem`), use that path; multiple roots can be provided via semicolon-separated list.
    - Note: Pointing `HTX_TRUST_PEM` at a leaf certificate that is not a CA will not satisfy rustls validation.
  - Fix (quick dev-only): Set `HTX_INSECURE_NO_VERIFY=1` in the Helper env to bypass TLS verification. Do not use outside local testing.

- Tip: On PowerShell, prefer the real curl executable when using `--socks5-hostname`:
  - `%SystemRoot%\System32\curl.exe` (PowerShell aliases `curl` to `Invoke-WebRequest`).

- Symptom: Status page hangs in a browser at `http://127.0.0.1:8088/status`
  - Cause: Your browser is likely sending localhost traffic through the Helper’s SOCKS proxy, creating a local hairpin via the edge (confirmed when the edge logs show a CONNECT to 127.0.0.1:8088). This adds latency and can keep the tab spinner active.
  - Fix: Bypass the proxy for localhost/127.0.0.1 when viewing the status page, or temporarily disable the browser/system proxy while checking `/status`. As an alternative, use the root URL `http://127.0.0.1:8088/` (it renders the JSON via `fetch('/status')`) or use `curl` for a quick, direct check.
  - Tip: For SOCKS verification, prefer an external domain (e.g., `example.com:80`) to avoid loopback hairpin: `%SystemRoot%\System32\curl.exe --socks5-hostname 127.0.0.1:1088 http://example.com/` (you should see the edge accept a CONNECT for `example.com:80`).
...
2) Start the Helper (stealth-browser) in masked mode with a status port
...
```powershell
%SystemRoot%\System32\curl.exe --socks5-hostname 127.0.0.1:1088 http://127.0.0.1:8088/status
```
...
```powershell
%SystemRoot%\System32\curl.exe http://127.0.0.1:8088/update
%SystemRoot%\System32\curl.exe --socks5-hostname 127.0.0.1:1088 http://127.0.0.1:8088/status
```
...
- [ ] Extension popup + options page (first‑run)
  - [ ] First‑run flow: detect Helper, guide install if missing; native messaging handshake
  - [ ] Status panel bound to Helper phases: downloading‑catalog → loading‑decoys → calibrating → ready
  - [ ] Retry/diagnostics if a phase fails; link to local logs; reflect seedless‑by‑default policy
- [ ] Proxy lifecycle controls
  - [ ] Start/Stop button wires to Helper (native messaging) and toggles browser proxy 127.0.0.1:1088
  - [ ] Settings: SOCKS port (default 1088), bootstrap toggle (dev), and decoy policy preset
- [ ] Catalog management (read‑only in MVP)
  - [ ] Display active signed catalog (bundled/cached/remote), version, expiry
  - [ ] Trigger update via Helper; surface last update result/time
- [ ] Connectivity health
  - [ ] Indicator: Connected / Calibrating / Offline
  - [ ] Retry button for failures; friendly error messages surfaced from Helper
- [ ] Logs
  - [ ] "Export logs" action fetches recent Helper logs for local download (no telemetry)

Exit: Usable for non-technical users with clear state and recovery.

### M5 — Packaging (Week 7)
- [ ] Helper: Windows MSI (WiX/NSIS), macOS PKG/DMG, Linux DEB/RPM with system service (optional)
- [ ] Extension: WebExtension builds for Chrome/Edge/Firefox; store materials and submission checklist
- [ ] Native messaging host manifest registration (per‑browser/platform)
...
### extension & helper scaffolding
- [x] Helper: `stealth-browser` runs SOCKS5 (127.0.0.1:1088) + status API (127.0.0.1:8088)
- [x] Extension skeleton (WebExtension) with popup and background scripts
- [x] Native messaging handshake (dev shim acceptable initially)
- [x] Basic request through SOCKS -> HTX echo validated via curl
...
### UX & controls
- [ ] Popup controls: Connect/Disconnect, status summary
- [ ] Options page: ports, bootstrap toggle (dev), decoy policy preset
- [ ] First‑run wizard + consent; Helper install detection
- [ ] Local‑only crash logs export button (via Helper logs)

### packaging & distribution
- [ ] Helper: code signing (dev), MSI/PKG/DEB/RPM installers
- [ ] Extension: WebExtension packaging and store submission (Chrome/Edge/Firefox)
- [ ] CI: build matrix per OS + webext; artifacts uploaded
...
## ci considerations
- Matrix builds for linux/windows/macos + webext
- Cache cargo + webext tooling/cache
- Jobs:
