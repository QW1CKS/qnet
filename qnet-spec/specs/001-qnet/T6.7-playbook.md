...
- Symptom: TLS error InvalidCertificate(Other(CaUsedAsEndEntity)) or server logs `AlertReceived(CertificateUnknown)`
  ...
  - Fix (preferred): Use a proper leaf certificate. The repo includes `certs/localhost.pem` and `certs/localhost-key.pem` which are suitable for dev on localhost. Start Edge with those and tell the Helper to trust that PEM:
    - Edge: `HTX_TLS_CERT=certs/localhost.pem`, `HTX_TLS_KEY=certs/localhost-key.pem`
    - Helper: set `HTX_TRUST_PEM` to `certs/localhost.pem` (semicolon-append if already set)
