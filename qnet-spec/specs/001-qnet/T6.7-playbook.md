...
Status: In progress (M1 complete; M2 in progress — stealth networking)
...
- M2 shaping landed behind feature flag:
...
- Pending in M2: decoy routing catalog + resolver, bootstrap resilience, DPI harness, expanded ALPN/JA3 fixtures and parity tests.
...
- [ ] TLS-like record sizing and padding profiles (small, webby, bursty)
  ...
  - [ ] Tests: distribution sanity (CDF buckets) for sizing profiles
- [ ] Timing jitter (bounded) for inter-record gaps
  ...
  - [ ] Tests: bound checks across profiles
- [ ] ALPN shaping and JA3-aligned client hello templates
  ...
  - [ ] Tests: JA3 parity vs reference fixtures; rotation cadence tests
...
- [x] Padding correctness tests (AEAD round-trip, back-compat decode); [ ] bounds/distribution CDF tests
- [x] Timing jitter with deterministic test mode
- [x] ALPN/JA3 selection + env allow-list rotation; [ ] parity fixtures + cadence policy
