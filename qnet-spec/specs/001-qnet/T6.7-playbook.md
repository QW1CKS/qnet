...
Status: In progress (M1 complete; M2 repo-local acceptance done: external seeds + DPI parity; QUIC parity evidence added; M3 catalog pipeline complete: signer CLI + signed artifacts verified; app-side loader/verify/cache/updater and status surfacing added; Windows validation successful; edge gateway added to M3 for production masked browsing)
...
- [ ] Edge gateway service (production masked browsing)
  - [ ] Server accept in htx (library): Implement htx::api::accept(bind) under rustls-config feature; mirror dial() for server-side TLS handshake, exporter EKM, inner mux.
  - [ ] Edge gateway binary: New `apps/edge-gateway` or `crates/edge-gateway`; accept outer TLS on 0.0.0.0:443 via htx::api::accept; for each inner stream, read CONNECT prelude, dial target over TCP, bridge bytes.
  - [ ] Certificates and catalog: Provision valid certs for decoy hostnames (e.g., Let's Encrypt); extend signed decoy catalog with edge entries pointing to your controlled hostnames.
  - [ ] Client handshake polish: Update masked mode to send HTTP CONNECT prelude on inner stream and wait for 200 before tunneling.
  - [ ] Verification: Smoke test curl over SOCKS → masked mode → hits edge → returns 200 CONNECT → page loads; Wireshark shows decoy-shaped outer TLS.

Exit: Catalog-first path working end-to-end (bundled+signed + updater + fallback) and surfaced in a minimal status API; edge gateway enables production masked browsing.
