...
#### Optional next steps (seedless online + prod hardening)
- Seedless “online” flip: document that the app sets state=connected after the first successful SOCKS CONNECT (no seeds required). Add a short note in `qnet-spec/docs/apps/stealth-browser.md`, including the Windows tip to use `%SystemRoot%\System32\curl.exe` through the local proxy to drive `/status` and verify the flip.
- Bundle real signed assets: include `assets/catalog-default.json` and `assets/catalog-default.json.sig` in the app bundle and enforce strict signature verification (remove `STEALTH_CATALOG_ALLOW_UNSIGNED` in prod). Ship the pinned publisher pubkey with the app.
...
- Troubleshooting docs: add a Windows note about PowerShell aliasing `curl` to `Invoke-WebRequest` and preferring `curl.exe` (or equivalent) when testing SOCKS.
