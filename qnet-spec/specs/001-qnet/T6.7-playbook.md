...
Status: In progress (M1 complete; M2 repo-local acceptance done: external seeds + DPI parity; QUIC parity evidence added; M3 catalog pipeline implemented: signer CLI + signed artifacts verified)
...
- [ ] Bundle catalog in app
  ...
  - [ ] Decide cache path (per-OS app data) and TTL enforcement policy
- [ ] Loader + verifier + persistence
  - [ ] Load cached (verify + fresh) → else bundled (verify); atomic write; keep last-known-good
  - [ ] Expose source (bundled/cached/remote), version, expiry via a small status API
- [ ] Updater from mirrors
  - [ ] Fetch from `update_urls` (GitHub raw/Pages/CDN); verify; compare version/expiry; replace cached atomically
