...
#[no_mangle]
pub unsafe extern "C" fn qnet_dial_inproc(
    out_client: *mut QnetConn,
    out_server: *mut QnetConn,
) -> c_int {
    let (c, s) = htx::api::dial_inproc_secure();
    let b1 = Box::new(c);
    let b2 = Box::new(s);
    if !out_client.is_null() {
        (*out_client).0 = Box::into_raw(b1);
    }
    if !out_server.is_null() {
        (*out_server).0 = Box::into_raw(b2);
    }
    0
}
...
pub unsafe extern "C" fn qnet_conn_open_stream(conn: *mut QnetConn) -> QnetStream {
    if conn.is_null() {
        return QnetStream(ptr::null_mut());
    }
    let c = &*((*conn).0);
    let s = c.open_stream();
...
/// `conn` must be a valid, non-null pointer to an initialized QnetConn.
#[no_mangle]
pub unsafe extern "C" fn qnet_conn_accept_stream(
    conn: *mut QnetConn,
    timeout_ms: u64,
) -> QnetStream {
    if conn.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn qnet_stream_write(
    st: *mut QnetStream,
    data: *const u8,
    len: size_t,
) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qnet_stream_read(
    st: *mut QnetStream,
    out_buf: *mut u8,
    cap: size_t,
) -> isize {
    if st.is_null() || out_buf.is_null() {
        return -1;
    }
    let s = &*((*st).0);
