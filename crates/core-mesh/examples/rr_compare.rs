...
mod with_libp2p {
    ...
    impl RrCodec for EchoCodec {
            ...
            'life2: 'async_trait,
        {
            Box::pin(async move {
                ...
                futures::io::AsyncWriteExt::write_all(io, &(data.len() as u32).to_be_bytes())
                    .await?;
    ...
    fn mk_rr_behaviour() -> EchoBehaviour {
        let cfg = request_response::Config::default().with_request_timeout(Duration::from_secs(10));
    ...
    async fn measure(proto: &str, n: usize, inflight: usize, sim: SimCfg) -> Result<Stats, String> {
        ...
        let server_transport = if proto == "tcp" {
            let ycfg = libp2p::yamux::Config::default();
            // Note: set_receive_window removed in newer libp2p versions
        ...
        let listen: Multiaddr = if proto == "tcp" {
            "/ip4/127.0.0.1/tcp/0"
                .parse()
        ...
        eprintln!(
            "[rr] starting server on {}",
            if proto == "tcp" { "tcp" } else { "quic" }
        );
        ...
        let (resp_tx, mut resp_rx) =
            mpsc::unbounded::<(request_response::ResponseChannel<String>, String, bool)>();
        ...
        'outer: loop {
            if start.elapsed() > timeout {
                return Err("timeout waiting for roundtrips".into());
            }
            futures::select! {
    ...
    pub async fn run() {
        ...
        while let Some(k) = it.next() {
            match k.as_str() {
                ...
                "--inflight" => {
                    if let Some(v) = it.next() {
                        inflight = v.parse().unwrap_or(inflight);
                    }
                }
                "--sim-rtt" => {
                    if let Some(v) = it.next() {
                        rtt_ms = v.parse().unwrap_or(rtt_ms);
                    }
                }
                "--sim-loss" => {
                    if let Some(v) = it.next() {
                        loss_pct = v.parse().unwrap_or(loss_pct);
                    }
                }
                _ => {}
            }
