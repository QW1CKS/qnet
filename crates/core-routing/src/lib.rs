...
pub enum Error {
    ...
    #[error("invalid structure")] Invalid,
}
...
pub struct Segment {
    pub version: u8,
    pub hops: Vec<Hop>,
    #[serde(with = "serde_bytes")] pub prev_sig: Vec<u8>, // aggregate or prev signer proof
}
...
pub struct SignedSegment {
    ...
    #[serde(with = "serde_bytes")] pub pubkey: Vec<u8>, // ed25519
}

impl Segment {
    pub fn new(version: u8, hops: Vec<Hop>, prev_sig: Vec<u8>) -> Self {
        Self { version, hops, prev_sig }
    }
    ...
    pub fn sign_ed25519(&self, seed32: &[u8; 32]) -> SignedSegment {
        ...
        SignedSegment { seg: self.clone(), sig, pubkey: pk }
    }
}
...
impl SignedSegment {
    pub fn verify(&self, now_ts: u64) -> Result<(), Error> {
        ...
        if self.seg.version != 1 { return Err(Error::Invalid); }
        if self.seg.hops.is_empty() { return Err(Error::Invalid); }
        // Timestamp/expiry bounds (check last hop window)
        let last = self.seg.hops.last().unwrap();
        if now_ts < last.ts { return Err(Error::Invalid); }
        if now_ts > last.ts.saturating_add(last.exp as u64) { return Err(Error::Invalid); }
...
mod tests {
    ...
    fn segment_sign_verify_and_bounds() {
        let hop = Hop { as_id: 42, if_in: 1, if_out: 2, ts: 1_700_000_000, exp: 600 };
