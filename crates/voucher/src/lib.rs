...
pub enum Error {
    #[error("invalid length: expected 128 bytes, got {0}")] InvalidLength(usize),
}
...
impl Voucher {
    pub fn from_bytes(b: &[u8]) -> Result<Self, Error> {
        if b.len() != 128 { return Err(Error::InvalidLength(b.len())); }
        let mut arr = [0u8;128];
        arr.copy_from_slice(b);
        Ok(Self(arr))
    }
    pub fn as_bytes(&self) -> &[u8;128] { &self.0 }
...
impl AggregatePlaceholder {
    pub fn add(&mut self, _v: &Voucher) { self.count = self.count.saturating_add(1); }
}
