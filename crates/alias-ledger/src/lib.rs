...
pub enum Error {
    #[error("sequence too old")] OldSeq,
    #[error("conflict not resolved")] Conflict,
    #[error("insufficient quorum")] NoQuorum,
}
...
impl Ledger {
    pub fn new(quorum: usize) -> Self { Self { quorum, ..Default::default() } }
    ...
    pub fn try_commit(&self, alias: Alias) -> Result<Option<Entry>, Error> {
        let mut p = self.pending.lock();
        let Some(by_seq) = p.get_mut(&alias) else { return Ok(None) };
